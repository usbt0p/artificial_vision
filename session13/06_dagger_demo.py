...
class DAggerAgent:
    ...
    def __init__(
        self, env, expert: ExpertPolicy, policy: DAggerPolicy, trainer: DAggerTrainer
    ):
        self.env = env
        self.expert = expert
        self.policy = policy
    ...
    def collect_initial_demonstrations(
    ...
    def collect_on_policy_data(
        self, num_episodes: int = 20, beta: float = 1.0
    ...
    def train_dagger(
        ...
        initial_states, initial_actions = self.collect_initial_demonstrations(
            initial_demos
        )
        ...
        for iteration in range(num_iterations):
            ...
            train_history = self.trainer.train(
                all_states, all_actions, num_epochs=training_epochs, batch_size=64
            )
            ...
            new_states, expert_labels, episode_returns = self.collect_on_policy_data(
                num_episodes=episodes_per_iter, beta=beta
            )
...
class Visualizer:
    ...
    def plot_training_curves(history: Dict, save_path: str = None):
        ...
        axes[4].set_title(
            "Expert Queries per Iteration", fontsize=12, fontweight="bold"
        )
        ...
        plt.suptitle(
            "DAgger Training Analysis", fontsize=15, fontweight="bold", y=0.995
        )
        plt.tight_layout()
        if save_path:
    ...
    def plot_learning_curves_comparison(
        dagger_history: Dict,
        bc_performance: float,
        gail_performance: float,
        ...
        ax.plot(
            dagger_history["iteration"],
            dagger_history["mean_return"],
            color="#3498db",
            linewidth=3,
            marker="o",
            markersize=8,
            label="DAgger (Iterative)",
            alpha=0.8,
        )
        ...
        ax.set_title(
            "DAgger Learning Curve vs Baselines", fontsize=15, fontweight="bold"
        )
...
def main():
    ...
    print(
        f"DAgger - Mean Return: {dagger_results['mean_return']:.2f} ± {dagger_results['std_return']:.2f}"
    )
    ...
    visualizer.plot_training_curves(
        dagger_history, save_path="dagger_training_curves.png"
    )
    ...
    print(
        f"  - DAgger: {dagger_results['mean_return']:6.2f} ± {dagger_results['std_return']:5.2f}"
    )
    print(
    ...
    dagger_vs_bc = (
        (dagger_results["mean_return"] - bc_results["mean_return"])
        / bc_results["mean_return"]
        * 100
    )
