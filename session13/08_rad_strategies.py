...
class RADAugmentations:
    ...
    def color_jitter(self, images: torch.Tensor, brightness: float = 0.1,
    ...
    def apply(self, images: torch.Tensor, aug_type: str) -> torch.Tensor:
        ...
        aug_map = {
            ...
            'color_jitter': lambda x: self.color_jitter(x, brightness=0.2, contrast=0.2),
...
class RADAgent:
    ...
    def __init__(self, env, input_channels: int, num_actions: int,
                 augmentation: RADAugmentations, aug_type: str = 'crop',
                 use_augmentation: bool = True, learning_rate: float = 1e-4):
        self.env = env
    ...
    def update(self, batch_size: int = 256, gamma: float = 0.99,
               num_augmentations: int = 2) -> Dict:
        """
    ...
    def train(self, num_steps: int = 30000, batch_size: int = 256,
              warmup_steps: int = 1000) -> Dict:
        """Train the RAD agent."""
        print(f"\n{'='*60}")
        print(f"Training with augmentation: {self.aug_type if self.use_augmentation else 'None'}")
        print(f"{'='*60}\n")
        ...
        for step in range(num_steps):
            ...
            if (step + 1) % 5000 == 0 and len(self.episode_returns) > 0:
                ...
                print(f"Step {step + 1:6d}/{num_steps} | Return: {mean_return:7.2f} | "
...
class Visualizer:
    ...
    @staticmethod
    def visualize_augmentations(env, augmentation: RADAugmentations, save_path: str = None):
        """Visualize all augmentation strategies."""
        obs, _ = env.reset()
        obs_array = np.array(obs)
        obs_tensor = torch.from_numpy(obs_array).unsqueeze(0).to(device)
        
        aug_types = ['none', 'crop', 'shift', 'cutout', 'cutout_color', 
                     'flip', 'color_jitter', 'grayscale']
    ...
    def plot_learning_curves(histories: Dict[str, Dict], save_path: str = None):
        ...
        for idx, (name, history) in enumerate(histories.items()):
            if 'mean_return' in history and len(history['mean_return']) > 0:
                ax.plot(history['step'], history['mean_return'],
                       label=name, color=colors[idx], linewidth=2, marker='o')
        
        ax.set_xlabel('Training Steps', fontsize=13)
        ax.set_ylabel('Mean Episode Return', fontsize=13)
        ax.set_title('Learning Curves: Augmentation Strategy Comparison', 
...
def main():
    ...
    visualizer.visualize_augmentations(env, augmentation, save_path='rad_augmentations.png')
    ...
    for aug_type in AUG_STRATEGIES:
        ...
        agent = RADAgent(env, input_channels, num_actions, augmentation,
                        aug_type=aug_type, use_augmentation=use_aug)
        
        # Train
        history = agent.train(num_steps=NUM_STEPS, batch_size=BATCH_SIZE,
    ...
    for rank, (name, results) in enumerate(sorted_results, 1):
        print(f"  {rank}. {name:15s}: {results['mean_return']:6.2f} ± {results['std_return']:5.2f}")
    ...
    improvement = ((best_aug[1]['mean_return'] - baseline) / baseline * 100) if baseline > 0 else 0
