...
class DomainRandomization:
    ...
    def randomize_brightness(
        self, obs: np.ndarray, intensity: float = 0.3
    ) -> np.ndarray:
        """
...
class DomainRandomizedEnv(gym.Wrapper):
    ...
    def _randomize_obs(self, obs):
        ...
        if len(obs_array.shape) == 2:
            ...
            randomized = self.domain_randomizer.randomize_brightness(
                randomized, self.brightness_intensity
            )
...
class Visualizer:
    ...
    def visualize_randomization_effects(
        env_canonical, env_randomized, save_path: str = None
    ):
        ...
        for i in range(3):
            ...
            axes[0, i + 1].set_title(
                f"Randomized {i+1}", fontsize=11, fontweight="bold"
            )
            axes[0, i + 1].axis("off")
    ...
    def plot_training_comparison(
        history_rand: Dict, history_no_rand: Dict, save_path: str = None
    ):
        ...
        # Returns
        axes[0].plot(
            history_no_rand["episode"],
            smooth(history_no_rand["return"]),
            color="#e74c3c",
            linewidth=2,
            label="No Randomization",
            alpha=0.8,
        )
        axes[0].plot(
            history_rand["episode"],
    ...
    def plot_transfer_comparison(results_dict: Dict[str, Dict], save_path: str = None):
        ...
        axes[0].set_title(
            "Transfer Performance Comparison", fontsize=14, fontweight="bold"
        )
...
def main():
    ...
    env_canonical = make_visual_env(
        ENV_NAME, IMG_SIZE, STACK_FRAMES, GRAYSCALE, randomize=False
    )

    # Randomized environment (training environment)
    env_randomized = make_visual_env(
        ENV_NAME, IMG_SIZE, STACK_FRAMES, GRAYSCALE, randomize=True
    )
    ...
    visualizer.visualize_randomization_effects(
        env_canonical, env_randomized, save_path="domain_randomization_effects.png"
    )
    ...
    print(
        f"Randomized → Canonical: {results_rand_to_canonical['mean_return']:.2f} ± "
        f"{results_rand_to_canonical['std_return']:.2f}"
    )

    # Policy trained without randomization, tested on canonical (sanity check)
    results_no_rand_to_canonical = trainer_no_rand.evaluate(
        NUM_EVAL_EPISODES, env_canonical
    )
    print(
        f"No Rand → Canonical:    {results_no_rand_to_canonical['mean_return']:.2f} ± "
    ...
    print(
        f"Randomized → Randomized: {results_rand_to_rand['mean_return']:.2f} ± "
        f"{results_rand_to_rand['std_return']:.2f}"
    )

    # Policy trained without randomization, tested on randomized (worst case)
    results_no_rand_to_rand = trainer_no_rand.evaluate(
        NUM_EVAL_EPISODES, env_randomized
    )
    print(
        f"No Rand → Randomized:   {results_no_rand_to_rand['mean_return']:.2f} ± "
    ...
    visualizer.plot_training_comparison(
        history_rand, history_no_rand, save_path="domain_rand_training.png"
    )
    ...
    visualizer.plot_transfer_comparison(
        transfer_results, save_path="domain_rand_transfer.png"
    )
    ...
    print(
        f"  Randomized → Real:    {results_rand_to_canonical['mean_return']:6.2f} ± {results_rand_to_canonical['std_return']:5.2f}"
    )
    print(
        f"  No Rand → Real:       {results_no_rand_to_canonical['mean_return']:6.2f} ± {results_no_rand_to_canonical['std_return']:5.2f}"
    )
    print(
        f"  Randomized → Rand:    {results_rand_to_rand['mean_return']:6.2f} ± {results_rand_to_rand['std_return']:5.2f}"
    )
    print(
        f"  No Rand → Rand:       {results_no_rand_to_rand['mean_return']:6.2f} ± {results_no_rand_to_rand['std_return']:5.2f}"
    )
    print()

    # Compute robustness metrics
    robustness_rand = min(
        results_rand_to_canonical["mean_return"], results_rand_to_rand["mean_return"]
    )
    robustness_no_rand = min(
        results_no_rand_to_canonical["mean_return"],
    ...
    print(
        f"  Improvement:           {((robustness_rand - robustness_no_rand) / robustness_no_rand * 100):.1f}%"
    )
    print()
