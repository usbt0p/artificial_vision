...
class RandomCropAugmentation:
    ...
    def _random_crop(
        self, padded_obs: torch.Tensor, batch_size: int, channels: int
    ) -> torch.Tensor:
        ...
        cropped = torch.zeros(
            ...
            self.crop_size,
            self.crop_size,
            device=padded_obs.device,
            dtype=padded_obs.dtype,
        )

        for i in range(batch_size):
            cropped[i] = padded_obs[
                i, :, h1[i] : h1[i] + self.crop_size, w1[i] : w1[i] + self.crop_size
            ]
...
class CURLModule(nn.Module):
    ...
    def __init__(
        self, input_channels: int, output_dim: int = 128, momentum: float = 0.99
    ):
    ...
    def update_key_encoder(self):
        ...
        for query_param, key_param in zip(
            ...
            key_param.data = (
                self.momentum * key_param.data + (1 - self.momentum) * query_param.data
            )
...
class CURLSACAgent:
    ...
    def update_curl(self, obs: torch.Tensor) -> Dict:
        ...
        stats = {"curl_loss": curl_loss.item(), "curl_accuracy": accuracy.item()}
...
class Visualizer:
    ...
    def plot_training_curves(
        curl_history: Dict, baseline_history: Dict = None, save_path: str = None
    ):
        ...
        if baseline_history:
            curl_final = (
                curl_history["mean_return"][-1]
                if len(curl_history["mean_return"]) > 0
                else 0
            )
            baseline_final = (
                baseline_history["mean_return"][-1]
                if len(baseline_history["mean_return"]) > 0
                else 0
            )

            improvement = (
                ((curl_final - baseline_final) / baseline_final * 100)
                if baseline_final > 0
                else 0
            )
            ...
            axes[5].set_title(
                f"Sample Efficiency (+{improvement:.1f}%)",
                fontsize=12,
                fontweight="bold",
            )
    ...
    def plot_representations(
        embeddings: np.ndarray, states: np.ndarray = None, save_path: str = None
    ):
        ...
        ax.set_title(
            "CURL Learned Representations (t-SNE)", fontsize=14, fontweight="bold"
        )
...
def main():
    ...
    curl_agent = CURLSACAgent(
        env, input_channels, num_actions, curl_latent_dim=CURL_LATENT_DIM
    )
    ...
    print(
        f"CURL - Mean Return: {curl_results['mean_return']:.2f} ± {curl_results['std_return']:.2f}"
    )
    ...
    visualizer.plot_representations(
        embeddings, states, save_path="curl_representations.png"
    )
    ...
    print(
        f"  - CURL: {curl_results['mean_return']:6.2f} ± {curl_results['std_return']:5.2f}"
    )
    print()
