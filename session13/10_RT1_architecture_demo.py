...
class ImageTokenizer(nn.Module):
    ...
    """
    def __init__(self, img_size: int = 84, patch_size: int = 14, 
...
class RT1Transformer(nn.Module):
    ...
    def __init__(self, embed_dim: int = 256, num_heads: int = 8,
                 num_layers: int = 4, num_actions: int = 2):
...
class RT1Policy(nn.Module):
    ...
    """
    def __init__(self, img_size: int = 84, patch_size: int = 14,
                 vocab_size: int = 100, embed_dim: int = 256,
                 num_actions: int = 2, max_instruction_length: int = 20):
    ...
    def forward(self, images: torch.Tensor, instructions: torch.Tensor,
    ...
    def get_action(self, image: np.ndarray, instruction: str,
                   instruction_encoder, deterministic: bool = False) -> int:
        """
        ...
        with torch.no_grad():
            ...
            instruction_tokens = instruction_encoder(instruction).unsqueeze(0).to(device)
...
class SimpleInstructionEncoder:
    ...
    def encode(self, instruction: str) -> torch.Tensor:
        ...
        if len(token_ids) < self.max_length:
            token_ids += [self.vocab.get('<PAD>', 0)] * (self.max_length - len(token_ids))
        else:
...
class RT1Trainer:
    ...
    """
    def __init__(self, env, policy: RT1Policy, instruction_encoder,
    ...
    def collect_episode(self, instruction: str) -> Tuple[List, List, List, float]:
        ...
        while not done:
            ...
            if np.random.random() > self.epsilon:
                action = self.policy.get_action(obs, instruction, 
    ...
    def train_on_episode(self, states: List, actions: List, rewards: List,
        ...
        instruction_batch = instruction_tokens.unsqueeze(0).repeat(len(states), 1).to(device)
    ...
    def evaluate(self, num_episodes: int = 10, instruction: str = "balance pole upright") -> Dict:
...
class Visualizer:
    ...
    def visualize_architecture(policy: RT1Policy, save_path: str = None):
        ...
        # Title
        fig.suptitle('RT-1 Architecture: Vision-Language-Action Transformer', 
                    fontsize=16, fontweight='bold')
        
        # Component 1: Image tokenization
        ax1 = fig.add_subplot(gs[0, 0])
        ax1.text(0.5, 0.5, 'Image\n(84×84×3)', ha='center', va='center',
        ...
        ax2.text(0.5, 0.5, f'Vision Tokens\n({num_patches} patches)', 
        ...
        ax4.text(0.5, 0.5, 'Multimodal Token Sequence\n[Vision Tokens | Language Tokens | Action Query]',
                ha='center', va='center', fontsize=12,
        ...
        ax5.text(0.5, 0.5, 'Transformer\nEncoder\n(4 layers)', ha='center', va='center',
                fontsize=12, bbox=dict(boxstyle='round', facecolor='plum'))
        ax5.set_xlim(0, 1)
        ax5.set_ylim(0, 1)
        ax5.axis('off')
        ...
        ax7.text(0.5, 0.5, 'Action\nLogits', ha='center', va='center',
                fontsize=12, bbox=dict(boxstyle='round', facecolor='orange'))
        ax7.set_xlim(0, 1)
        ax7.set_ylim(0, 1)
        ax7.axis('off')
    ...
    def plot_training_curves(history: Dict, save_path: str = None):
        ...
        # Returns
        axes[0].plot(history['episode'], smooth(history['return']),
        ...
        # Loss
        axes[1].plot(history['episode'], smooth(history['loss']),
    ...
    def visualize_tokenization(policy: RT1Policy, env, save_path: str = None):
        ...
        num_patches = policy.vision_tokenizer.num_patches
        axes[1].set_title(f'Patch Grid ({num_patches} patches, {patch_size}×{patch_size})', 
        ...
        axes[2].set_title(f'Vision Token Embeddings ({tokens_np.shape[0]}×{tokens_np.shape[1]})',
                         fontsize=12, fontweight='bold')
        axes[2].colorbar()
...
def main():
    ...
    for instruction in INSTRUCTIONS:
        results = trainer.evaluate(num_episodes=NUM_EVAL_EPISODES, instruction=instruction)
        print(f"'{instruction}': {results['mean_return']:.2f} ± {results['std_return']:.2f}")
