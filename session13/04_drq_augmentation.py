...
class ImageAugmentation:
    ...
    def random_crop(self, images: torch.Tensor) -> torch.Tensor:
        ...
        # Pad images
        padded = F.pad(images, (self.pad, self.pad, self.pad, self.pad), mode='replicate')
        ...
        for i in range(batch_size):
            cropped[i] = padded[i, :, h1[i]:h1[i]+self.img_size, w1[i]:w1[i]+self.img_size]
    ...
    def random_intensity(self, images: torch.Tensor, scale: float = 0.05) -> torch.Tensor:
        """
        ...
        intensity = 1.0 + scale * (2.0 * torch.rand(batch_size, 1, 1, 1, device=images.device) - 1.0)
...
class DrQTrainer:
    ...
    """
    def __init__(self, q_network: DrQNetwork, augmentation: ImageAugmentation,
                 num_actions: int, learning_rate: float = 1e-4,
                 gamma: float = 0.99, tau: float = 0.005,
                 num_augmentations: int = 2, aug_type: str = 'crop',
        ...
        self.target_q_network = DrQNetwork(
            q_network.encoder.conv1.in_channels,
            num_actions
        ).to(device)
...
class DrQAgent:
    ...
    """
    def __init__(self, env, q_network: DrQNetwork, trainer: DrQTrainer,
                 buffer_capacity: int = 100000, warmup_steps: int = 1000):
        self.env = env
        self.q_network = q_network
    ...
    def train(self, num_steps: int = 50000, batch_size: int = 256,
              update_freq: int = 1, eval_freq: int = 5000) -> Dict:
        """
        Train the DrQ agent.
        ...
        print(f"Augmentation: {self.trainer.aug_type if self.trainer.use_augmentation else 'None'}")
...
class Visualizer:
    ...
    def plot_training_curves(histories: Dict[str, Dict], save_path: str = None):
        ...
        for idx, (name, history) in enumerate(histories.items()):
            if 'mean_return' in history and len(history['mean_return']) > 0:
                axes[0].plot(history['step'], history['mean_return'], 
        ...
        axes[0].set_title('Episode Return vs Training Steps', fontsize=12, fontweight='bold')
        ...
        for idx, (name, history) in enumerate(histories.items()):
            if 'loss' in history and len(history['loss']) > 0:
                axes[1].plot(history['step'], history['loss'],
                           label=name, color=colors[idx % len(colors)], linewidth=2)
        ...
        for idx, (name, history) in enumerate(histories.items()):
            if 'q_values' in history and len(history['q_values']) > 0:
                axes[2].plot(history['step'], history['q_values'],
                           label=name, color=colors[idx % len(colors)], linewidth=2)
        ...
        for idx, (name, history) in enumerate(histories.items()):
            if 'epsilon' in history and len(history['epsilon']) > 0:
                axes[3].plot(history['step'], history['epsilon'],
                           label=name, color=colors[idx % len(colors)], linewidth=2)
    ...
    @staticmethod
    def plot_performance_comparison(results_dict: Dict[str, Dict], save_path: str = None):
        ...
        axes[0].bar(x_pos, means, yerr=stds, color=colors[:len(names)], alpha=0.7,
        ...
        for idx, name in enumerate(names):
            ...
            axes[1].hist(returns, bins=15, alpha=0.5, label=name, 
    ...
    @staticmethod
    def visualize_augmentations(env, augmentation: ImageAugmentation, save_path: str = None):
        ...
        for idx, aug_type in enumerate(aug_types):
            ...
            axes[0, idx].set_title(f'{aug_type.capitalize()}', fontsize=11, fontweight='bold')
            axes[0, idx].axis('off')
            ...
            axes[1, idx].set_title(f'{aug_type.capitalize()} (2)', fontsize=11, fontweight='bold')
            axes[1, idx].axis('off')
...
def main():
    ...
    visualizer.visualize_augmentations(env, augmentation, save_path='drq_augmentations.png')
    ...
    for name, use_aug, aug_type in configs:
        ...
        trainer = DrQTrainer(
            q_network, augmentation, num_actions,
        ...
        print(f"{name} - Mean Return: {results['mean_return']:.2f} ± {results['std_return']:.2f}")
    ...
    print(f"Random - Mean Return: {results_dict['Random']['mean_return']:.2f} ± {results_dict['Random']['std_return']:.2f}")
    ...
    visualizer.plot_performance_comparison(results_dict, save_path='drq_performance_comparison.png')
    ...
    for name in ['DrQ (Crop)', 'No Augmentation', 'Random']:
        if name in results_dict:
            print(f"  - {name:20s}: {results_dict[name]['mean_return']:6.2f} ± {results_dict[name]['std_return']:5.2f}")
    print()
    
    # Calculate improvement
    if 'DrQ (Crop)' in results_dict and 'No Augmentation' in results_dict:
        improvement = ((results_dict['DrQ (Crop)']['mean_return'] - 
                       results_dict['No Augmentation']['mean_return']) / 
                       results_dict['No Augmentation']['mean_return'] * 100)
