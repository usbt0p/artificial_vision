...
class ExpertDataCollector:
    ...
    def collect_trajectories(self, num_episodes: int = 100) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for episode in range(num_episodes):
            ...
            if (episode + 1) % 20 == 0:
                ...
                print(f"Episode {episode + 1}/{num_episodes} | Avg Return (last 20): {avg_return:.2f}")
        
        states = np.array(states_list)
        actions = np.array(actions_list)
        
        print(f"\nCollected {len(states)} state-action pairs")
        print(f"Expert average return: {np.mean(episode_returns):.2f} ± {np.std(episode_returns):.2f}")
        print(f"{'='*60}\n")
...
class PolicyEvaluator:
    ...
    def evaluate_policy(self, policy, num_episodes: int = 50, 
                       policy_name: str = "Policy", render: bool = False) -> Dict:
        """
        ...
        print(f"{policy_name} - Mean Return: {results['mean_return']:.2f} ± {results['std_return']:.2f}")
...
class DistributionShiftAnalyzer:
    ...
    def analyze_state_distribution(self, num_episodes: int = 10) -> Dict:
        ...
        for i, name in enumerate(state_names):
            print(f"{name:20s} | Expert: {analysis['expert_mean'][i]:7.3f} ± {analysis['expert_std'][i]:6.3f} | "
...
class Visualizer:
    ...
    def plot_performance_comparison(results_dict: Dict[str, Dict], save_path: str = None):
        ...
        for name, color in zip(policy_names, colors):
            ...
            axes[1].hist(returns, bins=20, alpha=0.5, label=name, color=color, edgecolor='black')
    ...
    def plot_distribution_shift(analysis: Dict, save_path: str = None):
        ...
        for i, name in enumerate(state_names):
            ...
            axes[i].hist(analysis['expert_states'][:, i], bins=50, alpha=0.6, 
                        label='Expert', color='#2ecc71', density=True, edgecolor='black')
            axes[i].hist(analysis['bc_states'][:, i], bins=50, alpha=0.6, 
                        label='BC Policy', color='#3498db', density=True, edgecolor='black')
            
            # Add mean lines
            axes[i].axvline(analysis['expert_mean'][i], color='#27ae60', 
                           linestyle='--', linewidth=2, label='Expert Mean')
            axes[i].axvline(analysis['bc_mean'][i], color='#2980b9', 
                           linestyle='--', linewidth=2, label='BC Mean')
        ...
        plt.suptitle('Distribution Shift Analysis: Expert vs BC Policy', 
...
def main():
    ...
    visualizer.plot_training_curves(training_history, save_path='bc_training_curves.png')
    ...
    visualizer.plot_performance_comparison(results_comparison, save_path='bc_performance_comparison.png')
    
    # Plot distribution shift
    print("Plotting distribution shift analysis...")
    visualizer.plot_distribution_shift(shift_analysis, save_path='bc_distribution_shift.png')
    ...
    print(f"  - Expert Policy:  {expert_results['mean_return']:6.2f} ± {expert_results['std_return']:5.2f}")
    print(f"  - BC Policy:      {bc_results['mean_return']:6.2f} ± {bc_results['std_return']:5.2f}")
    ...
    performance_ratio = (bc_results['mean_return'] / expert_results['mean_return']) * 100
