        ...
        for epoch in range(num_epochs):
            ...
            for start_idx in range(0, rollout_buffer.buffer_size, batch_size):
                ...
                with torch.no_grad():
                    ...
                    clip_fraction = (
                        ((ratio - 1).abs() > self.clip_epsilon).float().mean()
                    )
...
class PPOAgent:
    ...
    def __init__(
        self, env, policy: ActorCritic, trainer: PPOTrainer, rollout_steps: int = 2048
    ):
        self.env = env
        self.policy = policy
        ...
        for update in range(num_updates):
            ...
            if len(self.episode_returns) > 0:
                ...
                if (update + 1) % eval_freq == 0:
                    print(
                        f"Update {update + 1:3d}/{num_updates} | "
    ...
    def plot_training_curves(history: Dict, save_path: str = None):
        ...
        for idx, (key, title, color) in enumerate(metrics):
            if key in history and len(history[key]) > 0:
                axes[idx].plot(
                    history["update"], history[key], color=color, linewidth=2, alpha=0.8
                )
...
def main():
    ...
    print(
        f"PPO Policy - Mean Return: {ppo_results['mean_return']:.2f} ± {ppo_results['std_return']:.2f}"
    )
    ...
    visualizer.plot_comparison(
        ppo_results, random_results, save_path="ppo_performance_comparison.png"
    )
    ...
    print(
        f"  - PPO Policy:    {ppo_results['mean_return']:6.2f} ± {ppo_results['std_return']:5.2f}"
    )
    print(
    ...
    improvement = (
        (ppo_results["mean_return"] - random_results["mean_return"])
