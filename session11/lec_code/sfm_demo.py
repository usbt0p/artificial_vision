...
def recover_pose_from_essential(E, pts1, pts2, K):
    ...
    for R, t in solutions:
        ...
        pts_3d = triangulate_points_simple(pts1[:10], pts2[:10], 
                                           np.eye(3), np.zeros(3), R, t, K)
...
def visualize_sfm_results(points_3d_gt, points_3d_sfm, cameras, points_2d_views, 
                          inliers_list, save_path='sfm_demo_results.png'):
    """
    ...
    ax1.scatter(points_2d_views[0][:, 0], points_2d_views[0][:, 1], 
    ...
    ax2.scatter(points_2d_views[1][:, 0], points_2d_views[1][:, 1], 
    ...
    # Show inliers
    inliers = inliers_list[0] if len(inliers_list) > 0 else np.ones(len(points_2d_views[0]), dtype=bool)
    ...
    for i, (R, t) in enumerate(cameras):
        ...
        C = -R.T @ t
        ax4.scatter(C[0], C[1], C[2], c='black', s=100, marker='^', 
                   label=f'Camera {i+1}' if i < 2 else '')
        
        # Camera direction
        direction = R.T @ np.array([0, 0, 1])
        ax4.quiver(C[0], C[1], C[2], direction[0], direction[1], direction[2], 
                  length=0.5, color='gray', alpha=0.6)
    ...
    ax5.scatter(points_3d_sfm[:, 0], points_3d_sfm[:, 1], points_3d_sfm[:, 2], 
                c='red', s=30, alpha=0.6, label='SfM (before BA)')
    ...
    ax6.scatter(points_3d_sfm[:, 0], points_3d_sfm[:, 1], points_3d_sfm[:, 2], 
                c='red', s=30, alpha=0.6, label='SfM')
...
def main():
    ...
    F, inliers = estimate_fundamental_matrix_ransac(
        ...
        threshold=3.0, n_iterations=1000
    )
    ...
    scale = np.median(np.linalg.norm(gt_inliers, axis=1) / 
    ...
    scale_opt = np.median(np.linalg.norm(gt_inliers, axis=1) / 
    ...
    visualize_sfm_results(
        gt_inliers, points_3d_sfm_scaled, cameras_gt, 
        points_2d_views, [inliers]
    )
