...
def compute_depth_map_mvs(ref_image, support_images, R_ref, t_ref, 
                          support_cameras, K, depth_min=3.0, depth_max=7.0,
                          depth_samples=64, patch_size=7):
    """
        ...
        for x in range(patch_size, w - patch_size, step):
            ...
            for d_idx, depth in enumerate(depths):
                ...
                for (R_sup, t_sup), sup_image in zip(support_cameras, support_images):
                    ...
                    # Check bounds
                    if (patch_size <= x_sup_i < w - patch_size and 
                        patch_size <= y_sup_i < h - patch_size):
                        
                        # Extract support patch
                        sup_patch = extract_patch(sup_image, x_sup_i, y_sup_i, patch_size)
...
def depth_map_to_pointcloud(depth_map, confidence_map, image, R, t, K, 
...
def visualize_mvs_results(images, depth_maps, confidence_maps, 
                          sparse_points, dense_points, cameras,
    ...
    if len(sparse_points) > 0:
        ax1.scatter(sparse_points[:, 0], sparse_points[:, 1], sparse_points[:, 2],
                   c='blue', s=5, alpha=0.5, label='Sparse SfM')
    ...
    if len(dense_points) > 0:
        ...
        ax2.scatter(sample[:, 0], sample[:, 1], sample[:, 2],
                   c='green', s=1, alpha=0.3, label='Dense MVS')
    ...
    if len(sparse_points) > 0:
        ax3.scatter(sparse_points[:, 0], sparse_points[:, 1], sparse_points[:, 2],
                   c='blue', s=10, alpha=0.6, label=f'Sparse ({len(sparse_points)} pts)')
    
    if len(dense_points) > 0:
        step = max(1, len(dense_points) // 5000)
        sample = dense_points[::step]
        ax3.scatter(sample[:, 0], sample[:, 1], sample[:, 2],
                   c='green', s=1, alpha=0.2, label=f'Dense ({len(dense_points)} pts)')
...
def main():
    ...
    K = create_camera_matrix(focal_length=400, img_width=img_width, img_height=img_height)
    ...
    for i, (R, t) in enumerate(cameras):
        image, depth = render_textured_image(points_3d, texture, R, t, K, 
    ...
    sparse_indices = np.random.choice(len(points_3d), size=len(points_3d) // 20, replace=False)
    ...
    for i, (R_ref, t_ref) in enumerate(cameras):
        ...
        depth_map, confidence_map = compute_depth_map_mvs(
            images[i], support_images, R_ref, t_ref, support_cameras, K,
            depth_min=3.0, depth_max=7.0, depth_samples=32, patch_size=7
        )
    ...
    dense_points_3d = np.vstack(all_dense_points) if len(all_dense_points) > 0 else np.array([])
    print(f"  - Total dense points: {len(dense_points_3d)}")
    
    # Step 7: Visualize results
    print("\n[Step 7] Visualizing results...")
    visualize_mvs_results(
        images, depth_maps, confidence_maps,
        sparse_points_3d, dense_points_3d, cameras
    )
    ...
    print(f"  - Densification ratio: {len(dense_points_3d) / len(sparse_points_3d):.1f}x")
