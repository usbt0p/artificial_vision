...
def points_to_pillars(points, voxel_size, point_cloud_range, max_points_per_pillar=100):
    ...
    for (x, y), pts in pillar_dict.items():
        ...
        if features.size(0) < max_points_per_pillar:
            padding = torch.zeros((max_points_per_pillar - features.size(0), features.size(1)))
...
class KITTIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        points = torch.tensor(np.stack([x, y, z, intensity], axis=1), dtype=torch.float32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='./kitti',
                       help='Path to KITTI dataset')
    parser.add_argument('--class', type=str, default='car',
                       help='Object class to detect')
