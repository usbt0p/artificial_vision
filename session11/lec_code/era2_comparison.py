...
def plot_comparison_results(results, save_path='comparison_results.png'):
    ...
    for i, method in enumerate(methods):
        ...
        ax.plot(epochs, results[method]['train_acc'], 
               label=f'{method} (train)', color=colors[i], linestyle='-')
        ax.plot(epochs, results[method]['val_acc'], 
               label=f'{method} (val)', color=colors[i], linestyle='--')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Accuracy (%)')
...
def print_comparison_table(results):
    ...
    print("• Multi-View CNN typically achieves higher accuracy due to ImageNet pre-training")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Compare 3D ShapeNets vs Multi-View CNN')
    
    parser.add_argument('--demo', action='store_true',
                       help='Run in demo mode with synthetic results')
    parser.add_argument('--shapenets_model', type=str, default='shapenets_classifier.pth',
                       help='Path to trained 3D ShapeNets model')
    parser.add_argument('--multiview_model', type=str, default='multiview_cnn_best.pth',
