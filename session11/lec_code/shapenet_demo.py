...
class ShapeCompletion3D(nn.Module):
    ...
    def __init__(self, input_size=32):
        ...
        self.dec_conv1 = nn.ConvTranspose3d(512, 160, kernel_size=4, stride=2, padding=1)
        self.dec_bn1 = nn.BatchNorm3d(160)
        
        self.dec_conv2 = nn.ConvTranspose3d(160, 48, kernel_size=5, stride=2, padding=2, output_padding=1)
        self.dec_bn2 = nn.BatchNorm3d(48)
        
        self.dec_conv3 = nn.ConvTranspose3d(48, 1, kernel_size=6, stride=2, padding=2, output_padding=0)
...
def create_synthetic_voxel(shape_type='cube', resolution=32, class_id=0):
    ...
    if shape_type == 'cube' or class_id == 0:
        # Cube
        size = resolution // 3
        voxels[center-size:center+size, 
               center-size:center+size, 
               center-size:center+size] = 1.0
        ...
        voxels[center-5:center+5, center-5:center+5, center-5:center+5] = 1.0
...
class VoxelDataset(Dataset):
    ...
    """
    def __init__(self, num_samples=1000, resolution=32, num_classes=10, 
                 task='classification', occlusion_ratio=0.3):
...
def visualize_completion_results(partial, complete, predicted, save_path='completion_results.png'):
...
def main(args):
    ...
    if args.task == 'classification':
        ...
        with torch.no_grad():
            ...
            fig = visualize_voxels(voxel_np, 
                                  f'Class: {labels[0].item()}, Predicted: {predicted[0].item()}')
        ...
        for epoch in range(args.epochs):
            ...
            train_loss = train_completion(model, train_loader, optimizer, criterion, device)
