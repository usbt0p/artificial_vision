...
class IBVS:
    ...
    def compute_interaction_matrix(self, points_2d, depths):
        ...
        for i in range(N):
            ...
            L[2*i+1, :] = [0, -fy/Z, fy*y/Z, fy*(1+y**2), -fy*x*y, -fy*x]
...
def run_ibvs_simulation(visualize=True):
    ...
    T_initial = pose_matrix(
        tx=0.1, ty=-0.1, tz=0.8,  # Slightly offset
        rx=0.2, ry=-0.15, rz=0.1   # Slightly rotated
    )
    
    # Desired camera pose (directly facing target)
    T_desired = pose_matrix(
        tx=0.0, ty=0.0, tz=0.6,
    ...
    print(f"Initial pose: t=[{T_initial[0,3]:.3f}, {T_initial[1,3]:.3f}, {T_initial[2,3]:.3f}]")
    print(f"Desired pose: t=[{T_desired[0,3]:.3f}, {T_desired[1,3]:.3f}, {T_desired[2,3]:.3f}]")
    print()
    ...
    for i in range(max_iterations):
        ...
        # Compute control
        velocity, error = ibvs.compute_control(current_features, desired_features, depths)
    ...
    print()
    print(f"Final pose: t=[{T_current[0,3]:.3f}, {T_current[1,3]:.3f}, {T_current[2,3]:.3f}]")
    
    if visualize:
        visualize_ibvs_results(
            camera, target_points, desired_features,
            feature_history, error_history, pose_history
        )
    
    return error_history


def visualize_ibvs_results(camera, target_points, desired_features,
                           feature_history, error_history, pose_history):
    ...
    for point_idx in range(4):
        ...
        ax1.plot(traj[:, 0], traj[:, 1], '-', color=colors[point_idx], 
                alpha=0.5, linewidth=1)
        
        # Start point
        ax1.plot(traj[0, 0], traj[0, 1], 'o', color=colors[point_idx], 
                markersize=10, label=f'Start P{point_idx+1}')
        
        # End point
        ax1.plot(traj[-1, 0], traj[-1, 1], 's', color=colors[point_idx], 
                markersize=8)
        
        # Desired point
        ax1.plot(desired_features[point_idx, 0], desired_features[point_idx, 1],
                '*', color=colors[point_idx], markersize=15)
    ...
    ax3.scatter(target_points[:, 0], target_points[:, 1], target_points[:, 2],
               c='red', s=100, marker='s', label='Target')
    
    # Connect target points
    for i in range(4):
        j = (i + 1) % 4
        ax3.plot([target_points[i, 0], target_points[j, 0]],
                [target_points[i, 1], target_points[j, 1]],
                [target_points[i, 2], target_points[j, 2]], 'r-', linewidth=2)
    
    # Camera trajectory
    cam_positions = np.array([np.linalg.inv(T)[:3, 3] for T in pose_history])
    ax3.plot(cam_positions[:, 0], cam_positions[:, 1], cam_positions[:, 2],
            'b-', linewidth=2, label='Camera path')
    ax3.scatter(cam_positions[0, 0], cam_positions[0, 1], cam_positions[0, 2],
               c='green', s=100, marker='o', label='Start')
    ax3.scatter(cam_positions[-1, 0], cam_positions[-1, 1], cam_positions[-1, 2],
