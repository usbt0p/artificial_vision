...
def visualize_visual_dqn(agent, env, rewards_history, success_history):
    ...
    if len(success_history) >= window:
        success_rate = np.convolve(success_history, np.ones(window)/window, mode='valid') * 100
        ax2.plot(range(window-1, len(success_history)), success_rate, color='green', linewidth=2)
    ...
    if len(agent.loss_history) > 0:
        ...
        if len(agent.loss_history) >= window:
            smooth = np.convolve(agent.loss_history, np.ones(window)/window, mode='valid')
            ax3.plot(range(window-1, len(agent.loss_history)), smooth, 
