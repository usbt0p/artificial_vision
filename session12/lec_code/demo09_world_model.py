...
class SimpleCarEnv:
    ...
    def step(self, action):
        ...
        # Calculate reward
        dist_from_track = np.sqrt((self.x - self.track_center[0])**2 + 
                                  (self.y - self.track_center[1])**2)
    ...
    def _render(self):
        ...
        for angle in np.linspace(0, 2*np.pi, 100):
            for r in range(self.track_radius - self.track_width//2,
...
class MDNRNN(nn.Module):
    ...
    def forward(self, z, action, hidden=None):
        ...
        mu = self.fc_mu(lstm_out).view(*lstm_out.shape[:-1], self.n_gaussians, self.latent_dim)
        sigma = F.softplus(self.fc_sigma(lstm_out)).view(*lstm_out.shape[:-1], self.n_gaussians, self.latent_dim)
...
def mdn_loss(pi, mu, sigma, target):
    ...
    log_prob = -0.5 * ((target - mu) / sigma).pow(2) - torch.log(sigma) - 0.5 * np.log(2 * np.pi)
...
def train_mdnrnn(vae, mdnrnn, observations, actions, rewards, 
                 epochs=20, batch_size=16, seq_len=20, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for _ in range(len(observations) // batch_size):
            ...
            for ep_idx in ep_indices:
                ...
                obs_seq = np.array(ep_obs[start:start+seq_len+1]).astype(np.float32) / 255.0
...
def visualize_world_model(vae, mdnrnn, env, observations):
    ...
    sample_tensor = torch.FloatTensor(sample_obs).permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
    ...
    scatter = ax3.scatter(all_z[:, 0], all_z[:, 1], c=range(len(all_z)), 
    ...
    start_tensor = torch.FloatTensor(start_obs).permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
    ...
    # Show dream frames
    for i, (ax, idx) in enumerate(zip([axes[1, 0], axes[1, 1], axes[1, 2]], [0, 5, 10])):
...
def main():
    ...
    mdnrnn = MDNRNN(latent_dim=latent_dim, action_dim=3, hidden_dim=128, n_gaussians=3).to(device)
    ...
    # Train MDN-RNN
    rnn_losses = train_mdnrnn(vae, mdnrnn, observations, actions, rewards, 
                              epochs=30, batch_size=8, seq_len=15)
