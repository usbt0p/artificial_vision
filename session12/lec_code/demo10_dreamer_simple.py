...
class RSSM(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, hidden_dim=200, stoch_dim=30, deter_dim=200):
        super().__init__()
...
class SimpleDreamer:
    ...
    def __init__(self, action_dim, obs_dim=256, hidden_dim=200, 
                 stoch_dim=30, deter_dim=200, horizon=15):
        ...
        self.rssm = RSSM(obs_dim, action_dim, hidden_dim, stoch_dim, deter_dim).to(device)
    ...
    def select_action(self, obs, h, s, training=True):
        ...
        with torch.no_grad():
            ...
            _, s_new, _, posterior = self.rssm.observe(h, s, torch.zeros(1, self.action_dim).to(device), obs_embed)
            
            # Use posterior mean for deterministic state update
            h_new = self.rssm.gru(torch.cat([s, torch.zeros(1, self.action_dim).to(device)], dim=-1), h)
    ...
    def train_world_model(self, batch_size=16, seq_len=20):
        ...
        # Convert to tensors
        obs = torch.FloatTensor(np.array(batch_obs)).permute(0, 1, 4, 2, 3).to(device) / 255.0
    ...
    def train_actor_critic(self, batch_size=16):
        ...
        obs = torch.FloatTensor(np.array(batch_obs)).permute(0, 3, 1, 2).to(device) / 255.0
        
        # Encode initial observations
        with torch.no_grad():
            obs_embed = self.encoder(obs)
            h, s = self.rssm.initial_state(batch_size)
            _, s, _, _ = self.rssm.observe(h, s, torch.zeros(batch_size, self.action_dim).to(device), obs_embed)
            h = self.rssm.gru(torch.cat([s, torch.zeros(batch_size, self.action_dim).to(device)], dim=-1), h)
        ...
        for t in reversed(range(self.horizon)):
            returns[:, t] = rewards[:, t] + gamma * ((1 - lambda_) * values[:, t] + lambda_ * last_return)
...
def visualize_dreamer(agent, env, rewards_history):
    ...
    if len(agent.world_losses) > 0:
        ...
        if len(agent.world_losses) >= window:
            smoothed = np.convolve(agent.world_losses, np.ones(window)/window, mode='valid')
            ax2.plot(range(window-1, len(agent.world_losses)), smoothed, 'r-', linewidth=2)
