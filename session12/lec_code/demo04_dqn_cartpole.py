...
def visualize_training(rewards_history, epsilon_history, agent):
    ...
    if len(agent.loss_history) > 0:
        ...
        if len(agent.loss_history) >= window:
            smooth_loss = np.convolve(agent.loss_history, np.ones(window)/window, mode='valid')
            ax3.plot(range(window-1, len(agent.loss_history)), smooth_loss,
                    color='darkviolet', linewidth=2, label=f'Smoothed (window={window})')
    ...
    ax4.bar(x - width/2, q_values[:, 0], width, label='Action 0 (Left)', color='blue', alpha=0.7)
    ax4.bar(x + width/2, q_values[:, 1], width, label='Action 1 (Right)', color='red', alpha=0.7)
    ax4.set_xticks(x)
