...
class LandmarkWorld:
    ...
    def get_observations(self, robot_pose):
        ...
        for i, (lx, ly) in enumerate(self.landmarks):
            ...
            if true_range <= self.sensor_range and abs(true_bearing) <= self.sensor_fov / 2:
                # Add noise
                noisy_range = true_range + np.random.randn() * self.range_noise_std
                noisy_bearing = true_bearing + np.random.randn() * self.bearing_noise_std
    ...
    def update(self, observations, landmarks):
        ...
        for i in range(self.num_particles):
            ...
            for lm_id, obs_range, obs_bearing in observations:
                ...
                bearing_prob = np.exp(-0.5 * ((obs_bearing - expected_bearing) / 0.2)**2)
...
def run_particle_filter_demo():
    ...
    print(f"True initial pose: [{true_pose[0]:.1f}, {true_pose[1]:.1f}, {np.degrees(true_pose[2]):.1f}°]")
    print()
    ...
    for i, control in enumerate(tqdm(controls, desc="Localizing")):
        ...
        dx, dy, dtheta = control
        theta = true_pose[2]
        true_pose[0] += dx * np.cos(theta) - dy * np.sin(theta) + np.random.randn() * 0.05
        true_pose[1] += dx * np.sin(theta) + dy * np.cos(theta) + np.random.randn() * 0.05
    ...
    # Calculate errors
    position_errors = np.sqrt(np.sum((true_trajectory[:, :2] - estimated_trajectory[:, :2])**2, axis=1))
    
    print()
    print(f"Final true pose: [{true_pose[0]:.2f}, {true_pose[1]:.2f}, {np.degrees(true_pose[2]):.1f}°]")
    print(f"Final estimate: [{estimated_trajectory[-1, 0]:.2f}, {estimated_trajectory[-1, 1]:.2f}, {np.degrees(estimated_trajectory[-1, 2]):.1f}°]")
    ...
    visualize_particle_filter(
        world, true_trajectory, estimated_trajectory,
        particle_history, position_errors, n_eff_history
    )
    
    return pf


def visualize_particle_filter(world, true_traj, est_traj, particle_history,
                              errors, n_eff_history):
    ...
    # Landmarks
    ax1.scatter(world.landmarks[:, 0], world.landmarks[:, 1],
               c='red', s=150, marker='*', label='Landmarks', zorder=5)
    
    # Final particles
    final_particles = particle_history[-1]
    ax1.scatter(final_particles[:, 0], final_particles[:, 1],
               c='blue', s=5, alpha=0.3, label='Particles')
    
    # Trajectories
    ax1.plot(true_traj[:, 0], true_traj[:, 1], 'g-', linewidth=2,
            label='True trajectory', zorder=4)
    ax1.plot(est_traj[:, 0], est_traj[:, 1], 'b--', linewidth=2,
            label='Estimated trajectory', zorder=4)
    
    # Current positions
    ax1.scatter(true_traj[-1, 0], true_traj[-1, 1], c='green', s=200,
               marker='o', edgecolors='black', zorder=6, label='True pose')
    ax1.scatter(est_traj[-1, 0], est_traj[-1, 1], c='blue', s=200,
               marker='^', edgecolors='black', zorder=6, label='Estimated pose')
    ...
    ax3.axhline(y=len(particle_history[0]) / 2, color='r', linestyle='--',
    ...
    # Landmarks
    ax4.scatter(world.landmarks[:, 0], world.landmarks[:, 1],
    ...
    for t, color, label in zip(times, colors, labels):
        ...
        ax4.scatter(particles[:, 0], particles[:, 1], c=color, s=3, alpha=0.5, label=label)
