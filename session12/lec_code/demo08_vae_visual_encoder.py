...
def visualize_latent_space(vae, images, labels, n_samples=1000):
    ...
    for shape in range(3):
        ...
        ax1.scatter(z_2d[mask, 0], z_2d[mask, 1], c=colors[shape], 
                   label=shape_names[shape], alpha=0.5, s=20)
    ...
    ax2 = axes[1]
    scatter = ax2.scatter(z_2d[:, 0], z_2d[:, 1], c=all_positions[:, 0], 
    ...
    ax3 = axes[2]
    scatter = ax3.scatter(z_2d[:, 0], z_2d[:, 1], c=all_positions[:, 1], 
...
def visualize_latent_traversal(vae, images):
    ...
    ref_img = images[0]
    ref_tensor = torch.FloatTensor(ref_img).permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
    ...
    plt.suptitle('Latent Dimension Traversal\n(Each row varies one latent dimension)', fontsize=14)
    plt.tight_layout()
...
def visualize_interpolation(vae, images):
    ...
    img1_tensor = torch.FloatTensor(img1).permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
    img2_tensor = torch.FloatTensor(img2).permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
    ...
    plt.suptitle('Latent Space Interpolation\n(Smooth transition between two images)', fontsize=14)
    plt.tight_layout()
