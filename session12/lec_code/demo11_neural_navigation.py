...
class VisualNavigationEnv:
    ...
    def step(self, action):
        ...
        if action == 0:  # Turn left
            ...
            new_pos = self.agent_pos + 0.5 * np.array([
                np.cos(self.agent_angle),
                np.sin(self.agent_angle)
            ])
    ...
    def _render_egocentric(self):
        ...
        if abs(goal_angle) < self.fov / 2:
            ...
            img[max(0, y_center - marker_size):min(self.img_size, y_center + marker_size),
                max(0, x - marker_size):min(self.img_size, x + marker_size)] = self.colors['goal']
    ...
    def render_topdown(self):
        ...
        cell_size = 20
        img = np.full((self.grid_size * cell_size, self.grid_size * cell_size, 3),
                      self.colors['floor'], dtype=np.uint8)
...
class PPOTrainer:
    ...
    def __init__(self, policy, lr=1e-4, gamma=0.99, gae_lambda=0.95,
...
def collect_trajectory(env, policy, max_steps=100):
    ...
    for _ in range(max_steps):
        # Prepare observation
        image = torch.FloatTensor(obs['image']).permute(2, 0, 1).unsqueeze(0).to(device) / 255.0
...
def visualize_navigation(policy, env, rewards_history, success_history):
    ...
    if len(success_history) >= window:
        success_rate = np.convolve([float(s) for s in success_history], 
                                   np.ones(window)/window, mode='valid') * 100
        ax2.plot(range(window-1, len(success_history)), success_rate, 
    ...
    for _ in range(50):
        ...
        with torch.no_grad():
            action, _, _, hx, cx = policy.get_action(image, goal, hx, cx, deterministic=True)
