...
def analyze_overestimation(env_name='CartPole-v1', num_episodes=300):
    ...
    ax2.set_title('Q-value Estimates Over Training\n(Standard DQN tends to overestimate)')
    ax2.legend()
...
def visualize_dueling_architecture():
    ...
    ax2.bar(x - width/2, advantages[:, 0], width, label='Left (a=0)', color='blue', alpha=0.7)
    ax2.bar(x + width/2, advantages[:, 1], width, label='Right (a=1)', color='orange', alpha=0.7)
    ax2.set_xticks(x)
    ...
    ax3.bar(x - width/2, q_values[:, 0], width, label='Q(s, Left)', color='blue', alpha=0.7)
    ax3.bar(x + width/2, q_values[:, 1], width, label='Q(s, Right)', color='orange', alpha=0.7)
    ax3.set_xticks(x)
    ax3.set_xticklabels(state_labels, rotation=45, ha='right')
    ax3.set_ylabel('Q(s, a)')
    ax3.set_title('Q-values: Q(s,a) = V(s) + A(s,a)\n(Combines state value and action advantage)')
    ax3.legend()
    ...
    for i, label in enumerate(state_labels):
        ...
        print(f"  A(s, Left) = {advantages[i, 0]:.3f}, A(s, Right) = {advantages[i, 1]:.3f}")
        print(f"  Q(s, Left) = {q_values[i, 0]:.3f}, Q(s, Right) = {q_values[i, 1]:.3f}")
        print()
...
def compare_all_variants():
    ...
    colors = {'standard': 'red', 'double': 'blue', 'dueling': 'green', 'double_dueling': 'purple'}
    ...
    for variant, rewards in results.items():
        if len(rewards) >= window:
            ...
            ax.plot(smoothed, label=label, color=colors[variant], linewidth=2, alpha=0.8)
