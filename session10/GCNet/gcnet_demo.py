...
class GCNet(nn.Module):
    ...
    def build_cost_volume(self,
                          featL: torch.Tensor,
                          featR: torch.Tensor) -> torch.Tensor:
        """
    ...
    def soft_argmin(self, cost_volume: torch.Tensor) -> torch.Tensor:
        ...
        disp_values = torch.arange(D, device=cost_volume.device,
                                   dtype=cost_volume.dtype).view(1, D, 1, 1)
        disp_map = torch.sum(prob * disp_values, dim=1, keepdim=True)  # (B,1,H,W)
        return disp_map

    def forward(self,
                imgL: torch.Tensor,
                imgR: torch.Tensor) -> torch.Tensor:
        """
        ...
        disp_full = F.interpolate(disp_feat, size=(H, W),
...
def main():
    ...
    # ----- Train mode -----
    p_train = subparsers.add_parser("train", help="Train GCNet on synthetic stereo data")
    ...
    p_infer.add_argument("--output", type=str, default="gcnet_pred.png",
