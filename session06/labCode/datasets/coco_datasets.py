...
class COCODetectionDataset(Dataset):
    ...
    def __init__(
        self, img_dir, ann_file, transform=None, is_train=True, filter_empty=True
    ):
        """
        Args:
        ...
        self.coco_id_to_class = {
            coco_id: idx for idx, coco_id in enumerate(self.coco_ids)
        }
        self.class_to_coco_id = {
            idx: coco_id for coco_id, idx in self.coco_id_to_class.items()
        }

        print(
            f"Loaded COCO dataset: {len(self.img_ids)} images, {len(self.coco_ids)} classes"
        )
