...
def parse_args():
    ...
    parser.add_argument(
        "--conf-threshold", type=float, default=None, help="Confidence threshold"
    )
    parser.add_argument(
        "--nms-threshold", type=float, default=None, help="NMS IoU threshold"
    )
    parser.add_argument(
        "--save-results", action="store_true", help="Save detection results"
    )
    parser.add_argument(
        ...
        for images, boxes, labels, img_ids in tqdm(data_loader, desc="Evaluating"):
            ...
            if save_results or visualize:
                for img_id, dets in zip(img_ids, detections):
                    all_detections.append({"image_id": img_id, "detections": dets})
...
def main():
    ...
    # Set thresholds
    conf_threshold = (
        args.conf_threshold if args.conf_threshold else config.conf_threshold
    )
