...
class YOLOLoss(nn.Module):
    ...
    def forward(self, predictions, targets_boxes, targets_labels):
        ...
        loss_wh = (
            ...
            * torch.sum(
                ...
                * (
                    ...
                    - torch.sqrt(torch.clamp(target_wh.unsqueeze(3), min=1e-6))
                )
                ** 2
            )
            / batch_size
        ...
        loss_conf_noobj = (
            self.lambda_noobj
            * torch.sum(noobj_mask.unsqueeze(-1).unsqueeze(-1) * (pred_conf - 0) ** 2)
            / batch_size
        )
        ...
        loss_cls = (
            torch.sum(obj_mask.unsqueeze(-1) * (pred_cls_resp - target_cls) ** 2)
            / batch_size
        )
